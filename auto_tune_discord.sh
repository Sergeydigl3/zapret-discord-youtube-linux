...
declare -a WORKING_STRATEGIES=()  # Рабочие стратегии (номер:имя:ds_ok:cdn_ok)
TESTED_COUNT=0
SUCCESS_COUNT=0
FAILED_COUNT=0
...
# Извлечь номер из записи (формат: номер:имя:ds_ok:cdn_ok)
get_num_from_entry() {
    echo "${1%%:*}"
}
...
check_discord_main() {
    ...
    local code=$(curl -s --tlsv1.3 --connect-timeout "$CURL_TIMEOUT" --max-time "$CURL_TIMEOUT" \
        -o "$tmpfile" -w "%{http_code}" "https://www.discord.com" 2>/dev/null)
    local size=$(wc -c < "$tmpfile" 2>/dev/null || echo 0)
    local has_keyword=0
    grep -qi "discord" "$tmpfile" 2>/dev/null && has_keyword=1
    rm -f "$tmpfile"
...
check_discord_cdn() {
    local code=$(curl -s --tlsv1.3 --connect-timeout "$CURL_TIMEOUT" --max-time "$CURL_TIMEOUT" \
        -o /dev/null -w "%{http_code}" "https://cdn.discordapp.com" 2>/dev/null)
    [[ "$code" != "000" ]]
}
...
check_discord_full() {
    ...
    echo -n "  cdn.discordapp.com (CDN)... "
    if check_discord_cdn; then
        echo "✓"
        return 0
    else
        echo "✗"
...
# Тест стратегии, возвращает "ds_ok:cdn_ok"
test_strategy() {
    local ds_ok=0 cdn_ok=0
    if check_discord_main; then
        ds_ok=1
        check_discord_cdn && cdn_ok=1
    fi
    echo "$ds_ok:$cdn_ok"
}
...
────────────────────────────────────────────────────────────────"
    ...
    for entry in "${WORKING_STRATEGIES[@]}"; do
        ...
        local line=$(printf "  [%-2s] %-40s DS:✓ CDN:%s" "$num" "$name" "$cdn_status")
        echo "$line"
...
echo ""
echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║${NC}     ${BOLD}🔧 Auto Tune для zapret-discord${NC}                        ${BOLD}${CYAN}║${NC}"
echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# Проверка без zapret
echo "Проверяем доступность discord без zapret..."
if check_discord_full; then
    echo ""
    echo -e "${GREEN}✓${NC} Discord уже доступен. Ничего делать не нужно."
    exit 0
fi

        echo ""
echo "Discord недоступен. Тестируем стратегии..."
echo ""
...
for ((i=1; i<=MAX_STRATEGY; i++)); do
    ...
    if [[ $ds_ok -gt 0 ]]; then
        if [[ $cdn_ok -gt 0 ]]; then
            echo -e "${GREEN}DS:✓ CDN:✓${NC}"
        else
            echo -e "${GREEN}DS:✓${NC} ${RED}CDN:✗${NC}"
        fi
        ((SUCCESS_COUNT++))
        WORKING_STRATEGIES+=("$i:$name:$ds_ok:$cdn_ok")
    else
        echo -e "${RED}DS:✗${NC}"
        ((FAILED_COUNT++))
    fi
