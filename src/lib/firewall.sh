...
nft_setup() {
    ...
    if [[ "$ROUTER_MODE" == "1" ]]; then
        log "Включение IP forwarding (router mode)..."
        local sysctl_conf="/etc/sysctl.d/99-${SERVICE_NAME}.conf"
        elevate bash -c "echo 'net.ipv4.ip_forward=1' > \"$sysctl_conf\""
        elevate sysctl --system || handle_error "Ошибка применения sysctl конфигурации"
...
nft_clear() {
    ...
    # Удаляем sysctl конфиг если он был создан нами, и переприменяем
    local sysctl_conf="/etc/sysctl.d/99-${SERVICE_NAME}.conf"
    if [[ -f "$sysctl_conf" ]]; then
        elevate rm -f "$sysctl_conf" && log "Удалён $sysctl_conf" || log "Ошибка при удалении $sysctl_conf"
        elevate sysctl --system || log "Ошибка применения sysctl конфигурации"
    fi
