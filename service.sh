...
create_conf_file() {
    echo "Конфигурация отсутствует, неполная или редактируется. Перезаписываем новый конфиг."
...
# Запуск меню, если скрипт запущен пользователем и оповещение об использовании service.sh в качестве библиотеки
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    echo -e "Запуск методов из service.sh...\n"
fi
